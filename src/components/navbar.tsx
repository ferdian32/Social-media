...
import { onChangeEventHandler } from "../slice/inputSlice";
import { useSelector } from "react-redux";
import { useDispatch } from "react-redux";
export default function NavbarComponent () {
  const search = useSelector((state:RootState) => state.input.value);
  console.log(search);
  ...
  return (
    <nav className="w-full py-3 px-20 flex items-center gap-x-8 bg-black">
      ...
      <form action="" method="post" className="w-[400px] relative">
        <input type="search" name="search" value={search} id="search" placeholder="Masukan Pencarian ..." className="border border-gray-500 rounded-sm py-1 px-8 text-slate-50 w-full " onChange={(event:any) => dispatch(onChangeEventHandler(event.target.value))}/>
